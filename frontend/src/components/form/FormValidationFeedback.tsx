...
export const FormValidationFeedback: React.FC<FormValidationFeedbackProps> = ({
  ...
  const getSuggestion = (error: ValidationError): string | null => {
    switch (error.type) {
      ...
      case 'minLength':
        return t('form.validation.suggestions.minLength') || `Please enter at least ${error.constraint} characters.`;
      case 'maxLength':
        return t('form.validation.suggestions.maxLength') || `Please enter no more than ${error.constraint} characters.`;
      case 'pattern':
